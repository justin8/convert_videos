...
def test_video_settings_avc_nvidia():
    ...
    assert str(target) == " -map 0:v -map 0:s? -c:s copy -vcodec h264_nvenc -preset slow -rc constqp -qp 25 -b:v 0"


def test_video_settings_avc_intel():
    target = VideoSettings(Codec("AVC"), 25, "slow", encoder="intel")
    assert str(target) == " -map 0:v -map 0:s? -c:s copy -vcodec h264_qsv -preset slow -global_quality 25 -look_ahead 1 -g:v 120"
...
def test_video_settings_hevc_nvidia():
    ...
    assert str(target) == " -map 0:v -map 0:s? -c:s copy -vcodec hevc_nvenc -preset slow -rc constqp -qp 25 -b:v 0 -strict -2"


def test_video_settings_hevc_intel():
    target = VideoSettings(Codec("HEVC"), 25, "slow", encoder="intel")
    assert str(target) == " -map 0:v -map 0:s? -c:s copy -vcodec hevc_qsv -preset slow -global_quality 25 -g:v 120 -strict -2"


def test_video_settings_width():
    target = VideoSettings(codec=Codec("AVC"), quality=25, preset="slow", width=720, encoder="software")
    assert str(target) == " -map 0:v -map 0:s? -c:s copy -vcodec h264 -preset slow -crf 25 -vf scale=720:-2"
...
def test_video_language_filter_fullname():
    ...
    assert str(target) == " -map 0:v -map 0:s:m:language:eng? -c:s copy -vcodec h264 -preset slow -crf 25"


def test_video_language_filter_shortcode():
    target = VideoSettings(Codec("AVC"), 25, "slow", subtitle_language="jpn")
    assert str(target) == " -map 0:v -map 0:s:m:language:jpn? -c:s copy -vcodec h264 -preset slow -crf 25"
